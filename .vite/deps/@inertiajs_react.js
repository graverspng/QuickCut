import {
  require_react
} from "./chunk-IF2J66GS.js";
import {
  axios_default
} from "./chunk-F5OSY2WN.js";
import {
  __commonJS,
  __toESM
  ...
  "node_modules/object-inspect/index.js"(exports2, module2) {
    ...
    module2.exports = function inspect_(obj, options, depth, seen) {
      ...
      if (!isDate2(obj) && !isRegExp2(obj)) {
        ...
        var stringTag6 = !isPlainObject2 && toStringTag && Object(obj) === obj && toStringTag in obj ? $slice.call(toStr(obj), 8, -1) : protoTag ? "Object" : "";
...
function overRest(func, start3, transform2) {
  start3 = nativeMax4(start3 === void 0 ? func.length - 1 : start3, 0);
  return function() {
    var args = arguments, index = -1, length = nativeMax4(args.length - start3, 0), array = Array(length);
    while (++index < length) {
...
function baseFill(array, value, start3, end) {
  var length = array.length;
  start3 = toInteger_default(start3);
  if (start3 < 0) {
    start3 = -start3 > length ? 0 : length + start3;
  }
  ...
  end = start3 > end ? 0 : toLength_default(end);
  while (start3 < end) {
    array[start3++] = value;
  }
  return array;
...
function fill(array, value, start3, end) {
  ...
  if (start3 && typeof start3 != "number" && isIterateeCall_default(array, value, start3)) {
    start3 = 0;
    end = length;
  }
  return baseFill_default(array, value, start3, end);
}
...
function baseInRange(number, start3, end) {
  return number >= nativeMin6(start3, end) && number < nativeMax9(start3, end);
}
...
function inRange(number, start3, end) {
  ...
  number = toNumber_default(number);
  return baseInRange_default(number, start3, end);
}
...
function wrapperNext() {
  ...
  var done2 = this.__index__ >= this.__values__.length, value = done2 ? void 0 : this.__values__[this.__index__++];
...
function baseRange(start3, end, step, fromRight) {
  var index = -1, length = nativeMax13(nativeCeil4((end - start3) / (step || 1)), 0), result2 = Array(length);
  while (length--) {
...
function createRange(fromRight) {
  return function(start3, end, step) {
    if (step && typeof step != "number" && isIterateeCall_default(start3, end, step)) {
      end = step = void 0;
    }
    start3 = toFinite_default(start3);
    if (end === void 0) {
    ...
    step = step === void 0 ? start3 < end ? 1 : -1 : toFinite_default(step);
    return baseRange_default(start3, end, step, fromRight);
  };
}
...
function rest(func, start3) {
  ...
  start3 = start3 === void 0 ? start3 : toInteger_default(start3);
  return baseRest_default(func, start3);
}
...
function slice(array, start3, end) {
  ...
  if (end && typeof end != "number" && isIterateeCall_default(array, start3, end)) {
    start3 = 0;
    end = length;
  } else {
    start3 = start3 == null ? 0 : toInteger_default(start3);
    end = end === void 0 ? length : toInteger_default(end);
  }
  return baseSlice_default(array, start3, end);
}
...
function spread(func, start3) {
  ...
  start3 = start3 == null ? 0 : nativeMax14(toInteger_default(start3), 0);
  return baseRest_default(function(args) {
    var array = args[start3], otherArgs = castSlice_default(args, 0, start3);
    if (array) {
...
function trim(string, chars, guard) {
  ...
  var strSymbols = stringToArray_default(string), chrSymbols = stringToArray_default(chars), start3 = charsStartIndex_default(strSymbols, chrSymbols), end = charsEndIndex_default(strSymbols, chrSymbols) + 1;
  return castSlice_default(strSymbols, start3, end).join("");
}
...
function trimStart(string, chars, guard) {
  ...
  var strSymbols = stringToArray_default(string), start3 = charsStartIndex_default(strSymbols, stringToArray_default(chars));
  return castSlice_default(strSymbols, start3).join("");
}
...
var wrapperAt = flatRest_default(function(paths) {
  ...
  if (length > 1 || this.__actions__.length || !(value instanceof LazyWrapper_default) || !isIndex_default(start3)) {
  ...
  while (++index < length) {
    ...
    switch (data.type) {
      ...
      case "dropRight":
        ...
        break;
      case "take":
        end = nativeMin13(end, start3 + size2);
        break;
      case "takeRight":
        start3 = nativeMax16(start3, end - size2);
        break;
    }
  }
...
LazyWrapper_default.prototype.slice = function(start3, end) {
  start3 = toInteger_default(start3);
  var result2 = this;
...
function composeKey(parent2, key) {
  return parent2 ? parent2 + "[" + key + "]" : key;
}
...
var increaseByRandom = (amount) => {
  ...
  return set2(clamp2(n + amount, 0, 0.994));
};
var render = (fromStart) => {
  ...
  if (parent2 !== document.body) {
    parent2.classList.add(`${baseComponentSelector}-custom-parent`);
  }
...
function start2(event, delay2) {
  ...
  const timeout = setTimeout(() => progress_component_default.start(), delay2);
  ...
  const reactApp = await Promise.all([
    ...
    return setup({
      ...
      props: {
        ...
        resolveComponent,
        titleCallback: title,
        onHeadUpdate: isServer2 ? (elements) => head2 = elements : null
      }
    });
  });
...
function useForm(rememberKeyOrInitialValues, maybeInitialValues) {
  ...
  const [defaults2, setDefaults] = (0, import_react8.useState)(
  ...
  const [data, setData2] = rememberKey ? useRemember(defaults2, `${rememberKey}:data`) : (0, import_react8.useState)(defaults2);
  ...
  const isDirty = (0, import_react8.useMemo)(() => !isEqual_default(data, defaults2), [data, defaults2]);
  ...
  const setDataFunction = (0, import_react8.useCallback)(
    (keyOrData, maybeValue) => {
      if (typeof keyOrData === "string") {
        setData2((data2) => set_default(cloneDeep_default(data2), keyOrData, maybeValue));
      } else if (typeof keyOrData === "function") {
        setData2((data2) => keyOrData(data2));
      } else {
        setData2(keyOrData);
      }
    },
    ...
    (fieldOrFields, maybeValue) => {
      ...
      if (typeof fieldOrFields === "undefined") {
        ...
        setDefaults((defaults22) => {
          return typeof fieldOrFields === "string" ? set_default(cloneDeep_default(defaults22), fieldOrFields, maybeValue) : Object.assign(cloneDeep_default(defaults22), fieldOrFields);
        });
      }
    },
    [data, setDefaults]
...
var Form = (0, import_react7.forwardRef)(
  ({
    ...
    const getData2 = () => formDataToObject(getFormData());
    const updateDirtyState = (event) => setIsDirty(event.type === "reset" ? false : !isEqual_default(getData2(), formDataToObject(defaultData.current)));
